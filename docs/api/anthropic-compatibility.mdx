...
Make a local model available before use:
...
Cloud models are available immediately after signing in through the Windows app:
...
For tooling that relies on default Anthropic model names such as `claude-3-5-sonnet`, use `/api/copy` to copy an existing model name:
