...
Before using a model, make it available locally. For example:
...
For tooling that relies on default OpenAI model names such as `gpt-3.5-turbo`, use `/api/copy` to copy an existing model name to a temporary name:
...
Use `/api/create` to create a new model with the updated context size. Call the API with the updated model name:
