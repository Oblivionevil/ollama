...
For GitHub release packaging, `.github/workflows/release.yaml` accepts either the existing KMS signing configuration through `KEY_CONTAINER`, `OLLAMA_CERT_BASE64`, and `GOOGLE_SIGNING_CREDENTIALS`, or a PFX provided through `WINDOWS_SIGNING_PFX_BASE64` and `WINDOWS_SIGNING_PFX_PASSWORD`. The workflow uses `APPINSTALLER_BASE_URI` when configured, and otherwise points the generated `.appinstaller` manifest at the GitHub release asset URL for the current tag.
